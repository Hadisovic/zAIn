...
export function ChatWidget() {
  const setMainView = useConfigStore((s) => s.setMainView)
  const isProcessing = useChatStore((s) => s.isProcessing)
  const latestRequestId = useChatStore((s) => s.latestRequestId)

  // Resize window on mount/unmount
  useEffect(() => {
    const expand = async () => {
      try {
        ...
        console.error('[ChatWidget] Expand geometry failed:', e)
      }
    }
    expand()
    ...
    return () => {
      const collapse = async () => {
        try {
          ...
          console.error('[ChatWidget] Collapse geometry failed:', e)
        }
      }
      collapse()
    }
